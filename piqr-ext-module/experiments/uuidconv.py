...
from uuid_qr import qr_alphanum_to_uuid, uuid_to_qr_alphanum
...
u = uuid.UUID('557b78a6f2124155a40a032433d5b6c6')
print(u.bytes)
print(u.int, type(u.int))

# BASE64_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
# BASE64_TO_BYTE = {sym: i for i, sym in enumerate(BASE64_ALPHABET)}
# CODE128_ALPHABET = ' !"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_'
# CODE128_TO_BYTE = {sym: i for i, sym in enumerate(CODE128_ALPHABET)}
#
# print(BASE64_TO_BYTE)
#
# for i, b in enumerate(u.bytes):
#     print(f"[{i}] %x {int(b)}" % int(b))
#
# b64 = b64encode(u.bytes).decode('ascii').strip('=')
# print(b64, len(b64))
#
# def base64_to_code128(s):
#     return ''.join([CODE128_ALPHABET[BASE64_TO_BYTE[ch]] for ch in s])
#
# c128 = base64_to_code128(b64)
# print(c128)
#
# code128.image(c128).save("code128.png")


aln = uuid_to_qr_alphanum(u)
print(f"Alnum: {aln}")
print(f"Back to UUID: {qr_alphanum_to_uuid(aln)}")
print(qr_alphanum_to_uuid(aln) == u)

c128 = code128_format(str(u.int))
print(c128)

img = code128_image(c128, height=400, thickness=8)
with open("code128-1.png", "wb") as f:
    img.save(f, "PNG")

code128.image(str(u.int)).save("code128-2.png")


qr = qrcode.QRCode(error_correction=qrcode.ERROR_CORRECT_H, box_size=10)

# logo = Image.open('logo-transp.png')
logo = Image.open('../key_manager/static/img/logo.jpg')

qr.add_data(uuid_to_qr_alphanum(u))
